...
- **Bulk-clean stale branches.** `wt prune --all` (`-a`) deletes every local branch
  ...
  - `--merged`: worktrees and branches merged into the default branch, either the
    ...
    `merge=ours`) and the built-in `merge=union` driver are disabled for this
    check, so a file one governs reads as not merged, and `merge.default` is
    treated as `text`. The content check needs git ≥ 2.38. On older git only
    ancestry counts.
